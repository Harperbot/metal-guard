...
**Current version: v0.11.4** — see [CHANGELOG.md](CHANGELOG.md) for release history and per-feature rationale.
...
```bash
metal-guard --version          # → metal-guard 0.11.4
...
curl -L -o ~/lib/metal-guard/metal_guard.py \
  https://raw.githubusercontent.com/Harperbot/metal-guard/v0.11.4/metal_guard.py
```
...
`pip install "git+https://github.com/Harperbot/metal-guard.git@v0.11.4"` or
